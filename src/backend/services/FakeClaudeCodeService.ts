...
export class FakeClaudeCodeService implements ClaudeCodeService {
	...
	private async handleUserMessage(content: string) {
		...
		this.stdoutController.enqueue(encoder.encode(`${JSON.stringify(initMessage)}\n`));

		// 2. Determine response based on content
		const response = this.responseMap.get(content) || `Echo: ${content.substring(0, 50)}`;
		...
		this.stdoutController.enqueue(encoder.encode(`${JSON.stringify(assistantMessage)}\n`));
		...
		this.stdoutController.enqueue(encoder.encode(`${JSON.stringify(resultMessage)}\n`));
	}
